...
const fs = require('fs')
const QRCode = require('@andrewiski/qrcode')
...
app.get('/qrcode.js', (req, res) => {
  ...
  fs.createReadStream(path.join(__dirname, 'node_modules', '@andrewiski', 'qrcode', 'build', 'qrcode.js')).pipe(res)
})

app.get('/qrcode.tosjis.js', (req, res) => {
  res.set('content-type', 'text/javascript')
  fs.createReadStream(path.join(__dirname, 'node_modules', '@andrewiski', 'qrcode', 'build', 'qrcode.tosjis.js')).pipe(res)
})
